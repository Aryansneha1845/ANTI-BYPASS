...
SERVER_URL = "https://anti-bypass.onrender.com"  # Apna sahi Render URL bina slash ke rakhna

bot = telebot.TeleBot(BOT_TOKEN)
app = FastAPI()
templates = Jinja2Templates(directory="templates")

# Link ko safe string mein convert karne ke liye helper functions
def encode_url(url: str) -> str:
    return base64.urlsafe_b64encode(url.encode()).decode().rstrip("=")

def decode_url(encoded_str: str) -> str:
    try:
        # Padding fix karne ke liye
...
def start_command(message):
    bot.reply_to(message, "👋 **Welcome!** Apne shortener link (Arolinks, Vplinks, etc.) bhejo, main use bypass-proof bana dunga.", parse_mode="Markdown")

@bot.message_handler(func=lambda message: message.text.startswith(('http://', 'https://')))
def process_link(message):
    original_url = message.text.strip()
    
    # Bina database ke link ko encode kiya
...
async def serve_verify_page(request: Request, link_id: str):
    # Check ki link decode ho pa raha hai ya nahi
    original_url = decode_url(link_id)
    if not original_url:
        return HTMLResponse(content="<h3>Error: Invalid or Corrupted Link!</h3>", status_code=400)
...
async def handle_verification(request: Request, link_id: str):
    ...
    # Link ko wapas decode karke redirect kiya
    original_url = decode_url(link_id)
    if original_url:
        return RedirectResponse(url=original_url, status_code=303)
        
    return HTMLResponse(content="<h3>Link Not Found or Expired!</h3>", status_code=404)
